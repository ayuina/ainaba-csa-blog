...
ざっくり言えば１つのリージョンを最低３つの区画に分けることで、地震、水害、火災といったような局所的な災害においてもリージョン全体が機能不全に陥らないようにしたものです。クラウドサービスとは言っても最終的にはどこかのデータセンターに物理的に配置されたサーバー上で動作しているソフトウェアに過ぎませんから、上記のような災害やあるいは大規模障害が発生すれば一時的な機能停止は避けられません。

一方ユーザーから見ると多くの Azure サービスは`東日本リージョン`などのようにリージョン単位で提供されているわけで、その最終的な配置先を制御することはできません。
つまり災害や障害が発生しないように、あるいは発生しても影響が無いようにお祈りするしか無いことになりますし、リージョンの（全てではなく）一部が被災や障害で利用不能な状態になった時でも、正常な区画を利用してサービスは継続して欲しいわけです。

このため複数の可用性ゾーンに`分散配置`することで、障害や災害時でもサービスを続行できる高可用性を得るためのオプションが提供されているわけです。
ここまで何度もオプションであることを強調しているのは、Azure が勝手にやってくれるわけではなく、ユーザーが選択する必要があるからです。
