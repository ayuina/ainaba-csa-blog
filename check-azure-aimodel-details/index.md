...
データとしては膨大な量になりますし、Azure Resource Manager に過度な負荷をかけるのも申し訳ないので、
各リージョン単位でファイル出力しておき、その後はファイルの情報を元に解析をしていくこととします。

なお json をひたすら処理することになるのでお好みのプログラミング言語を利用すれば良いのですが、
私は PowerShell が好きなので、以下のサンプルは PowerShell から Azure CLI を実行しています。
...
ただ Capability など、「何となく意味はわかるものの、明確な定義の記載が見つからない」ものがありますので、詳細が気になる方は Azure サポートにお問い合わせいただくと良いのかなと思います。
