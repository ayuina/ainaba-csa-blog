...
複雑怪奇なセキュリティルールにより Azure Potal や Azure Active Directory の一部あるいは全部にアクセスできない、ということがたまによくあります。
...
ASP.NET Core なら `dotnet publish` コマンドで生成できます。（.NET Core って最近は言わないんでしょうけど紛らわしいので）
従来の ASP.NET なら Visual Studio の発行ウィザードで、出力先でフォルダーを選択してやれば OK です。
ここも自動化できてないと不味いだろう、というかたは Visual Studio で発行プロファイルが出来上がっていると思うので、
そちらを指定して [msbuild.exe を使用して発行](https://qiita.com/okazuki/items/1227dd8ba92e45fbc50c)して上げると良いでしょう。

それ以外の開発環境や言語の場合は適宜ドキュメント等をご参照ください。
