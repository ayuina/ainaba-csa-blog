...
```powershell
$clientid = "guid of client id"
$tenantid = "guid of tenant id"
```
...
```powershell
$password = "secret string generated by portal"
```
...
＃実行結果として RBAC によって明示的にアクセス権を付与したリソース（この場合はVNET）だけが表示されるはず
```
...
＃Azure Resource Manager を利用するためのアクセストークンを取得
$resource = "https://management.azure.com"
$api_version = "2018-02-01"
$url = "http://169.254.169.254/metadata/identity/oauth2/token?api-version=${api_version}&client_id=${clientid}&resource=${resource}"
$response = Invoke-WebRequest -UseBasicParsing -Method Get -Headers @{Metadata=$true} -Uri $url
$token = $response.Content | ConvertFrom-Json

＃取得したアクセストークンを用いてAzure に接続
Connect-AzAccount -AccessToken $token.access_token -AccountId $clientid
