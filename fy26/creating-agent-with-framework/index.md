...
いきなりイレギュラーな構成な感じもありますが、個人的には割と重要なポイントかなと思っています。
...
// 内部的に Chat Completion を使用するクライアント
...
そしてエージェントを開発するためにプログラミングを行うのですが、「最終的に AsAIAgent を使用する」という点は一緒です。
...
このケースで考えられるのは、オンプレミス環境に存在する機微なデータやそれを扱う API を使用したい、とはいえモデルは（ローカルで動くような SLM ではなく）クラウドで動作する高機能な LLM を使用したい、というような時でしょうか。

LLM がクラウドにあるということは、通信経路的には機微なデータが企業の環境から外部には出てしまいます。
データの扱いとして TLS と Entra ID 認証だけではセキュリティ要件を満たせない、という場合には Express Route や VPN 等を使用して閉域化することが可能です。
...
![alt text](./images/agent-on-aspnet.png)
...
雑な作りではありますが、これを実行してホストされた http://localhost:5000 をブラウザ等で開く（HTTP GET）すると以下のような回答が返ってきます。
機能的にはほとんど変えていないので、関西弁で現在時刻を教えてくれるだけですね。
...
前述の図のようにオンプレのサーバーで動かしてもいいですし、Azure App Sercice や Azure Container Apps 上にホストして運用負荷を軽減するのもよいでしょう。
...
さて様々な連携先との相互運用性を考える場合には、エージェントとの対話のプロトコルが標準化されていることが重要です。
...
![hosting adapter](./images/agent-with-hosting-adapter.png)
...
サンプルコードを見ると長々と書いてあるのですが、実は適用自体は簡単で、
...
// プロトコル クライアントのラッパーとなる AIAgent を作成する流れは一緒
var chatClient = GetOpenAIChatClient();
var agent = CreateAgent(chatClient);

// 作成したエージェントをホスティング アダプターを使用してホスト（実質これ）
...
エージェントと Responses API で対話できていることですね。
...
![alt text](./images/deploy-as-hosted-agent.png)

#### エージェントアプリのコンテナ化

まずはホスティング アダプターを使用してエージェントをホストするアプリをコンテナでくるんであげる必要があります。
ビルドコンテナで `dotnet publish` しておいて実行可能ファイルを runtime コンテナに持ち込むのが正しいんでしょうが、ここは簡易的に以下の Dockerfile のように SDK ごと持ち込む実装とします。
...
これは Azure CLI を使用して `az login` した際の認証情報キャッシュを使っているので、Foundry 等のクラウド PaaS 上では動作しません。
...
このあと Docker を使用してコンテナ イメージをビルドして動作確認したいのですが、ここでも Credential 問題が付きまといます。
`docker run` して動作させたコンテナの中にどうやって Azure のクレデンシャルを持ち込むか。
~~もう面倒なので~~ そこは本題ではないので、もうこのままテストせずにコンテナをぶち込んでいきます。
...
ここの例ではエージェントは頭脳として Foundry Model を使用していますので、当然そこにアクセス可能である必要があります。
（外部の LLM を使用しているならまた違った別の方式になるかと）

開発中の（未発行の）全てのエージェントはクレデンシャルとしてプロジェクト レベルのマネージド ID を利用します。
つまりこの ID が LLM（Foundry Models）にアクセス可能であればよいので、「プロジェクト レベルのマネージド ID に対して、Foundry の Azure AI User ロールを割り当て」ておきましょう。

### ホステッドエージェントの動作確認

さてやっと出来たホステッドエージェント、折角なので動かしてみたいですよね。
...
#### [補足] エージェントの発行によって何が起こるのか

`az cognitive service agent create` しただけのエージェントは「発行」されておらず、あくまでも「開発中の資産」という扱いになります。
エージェントの発行によって何が起こるか、詳細は[ドキュメントを参照](https://learn.microsoft.com/ja-jp/azure/foundry/agents/how-to/publish-agent)いただければと思いますが、
ざっくり言えば以下のような処理が行われます。

- Azure Portal には表示されないエージェント アプリケーションとエージェント デプロイメント というリソースが生成される
- エージェント アプリケーションは専用のエンドポイントを持ち、エージェント デプロイメントに対して処理を転送する構成になっている
- デプロイメント自体もエージェントの定義情報は持っておらず、エージェント作成時につけられたバージョンを参照している
- コンテナイメージの初回作成時は上記の構造が作成されるが、2回目以降のバージョンアップ時には転送先は古い方を向いたまま
- 発行処理をして初めてエージェント デプロイメントの向き先が切り替わる

つまりホステッド エージェントのコンテナのイメージをプッシュして、新しいバージョンアップ作成しただけでは、運用中のエージェントは新しくなりません。
これはプロンプトベースのエージェントと同じ挙動ですね。
未発行状態のバージョンに対して最終的なテストを行ったのちに、**発行** プロセスを経て公開に至るわけです。

エージェントが発行されると、以下のような特定のエージェント専用のエンドポイントが払い出されます。
発行後はこちらのエンドポイントを使用して **Response API** で対話できるというわけです。
 
|エンドポイント|URL|
|---|---|
|プロジェクト|https://{foundry}.services.ai.azure.com/api/projects/{project}|
|発行したエージェント|https://{foundry}.services.ai.azure.com/api/projects/{project}/applications/{agent}/protocols/openai/responses|

#### [補足] Entra ID への登録と Agent ID の生成

エージェントの発行によって Microsoft Foundry に登録されるだけでなく、Entra ID にも登録が行われています。
上記のようなエージェント開発を進めて公開した後に Entra ID を見ると以下のようになっています。
名前が似通ってて区別がつきにくいのですが、

- 一番上の１つめは Project が作成した時点で付与される Managed ID を表しています（生成日は Project を作成した日
...
その解決策の１つとして提供されているのが Foundry と Entra ID ということになります。
作りとしてはオープンな標準（お作法？）に則りつつ、管理性は犠牲にせず、Foundry や Azure をフル活用して開発生産性を最大化できるといいなと思っています。
